...
class QuestGame:
    ...
    def draw(self) -> None:
        ...
        # --- Niebla de guerra adaptada a paredes (FOV simple) ---
        ...
        for angle in range(0, 360, 2):  # 2 grados por paso
            rad = angle * 3.14159 / 180
            for dist in range(0, vision_radius, 4):  # Paso de 4 píxeles
                ray_x = dist * pygame.math.Vector2(1, 0).rotate(angle).x
                ray_y = dist * pygame.math.Vector2(1, 0).rotate(angle).y
                x = int(hero_screen_pos[0] + ray_x)
                y = int(hero_screen_pos[1] + ray_y)
                # Convertir a coordenadas de mapa
                map_x, map_y = (x, y)
                # Chequear colisión con paredes
                if any(wall.collidepoint(map_x, map_y) for wall in self.walls):
                    break  # Detener el rayo si choca con una pared
                # "Limpiar" la niebla en este punto
                pos = (ray_x+400, ray_y+300)
                pygame.draw.circle(fog, (0, 0, 0, 0), pos, 4)
